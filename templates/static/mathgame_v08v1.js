...
function convertMap(mapAsString) {
  ...
  return {map, mapOrig, width, height, chests};
}
...
function postBuy($cell, amount, chest = null) {
  postData('/game/buy', {x: $cell.coln, y: $cell.rown, amount, chest})
...
function openRandomCell($cell) {
  const amount = 0;
  const {coln, rown} = $cell;
  let ncoln, nrown, $ncell;
  for (let i_ = 0; i_ < 1000; i_++) {
    ncoln = coln + Math.round(-RANDOM_DIAM + (2 * RANDOM_DIAM + 1) * Math.random());
    nrown = rown + Math.round(-RANDOM_DIAM + (2 * RANDOM_DIAM + 1) * Math.random());
    if (ncoln <= 0 || ncoln >= scene.width - 1 || nrown <= 0 || nrown >= scene.height - 1) {
      continue;
    }
    // debugger;
    $ncell = scene.$cells[nrown][ncoln];
    if (($ncell.textContent === '' || /^\d+$/.test($ncell.textContent)) && /^\d+$/.test(scene.mapOrig[nrown][ncoln])) {
      break;
    }
  }
  if (!(($ncell.textContent === '' || /^\d+$/.test($ncell.textContent)) && /^\d+$/.test(scene.mapOrig[nrown][ncoln]))) {
  ...
  // Нашли ячейку для открытия
  $ncell.textContent = "...";
  $ncell.onclick = null;
  console.log(`Пытаемся открыть ячейку ${$ncell.coln} ${$ncell.rown} за ${amount}`);
  if (!DEBUG) {
    postBuy($ncell, amount, 'chest');
  } else {
    succesBuy($ncell);
  }
}
...
function showChestSecret(ev) {
  ...
  showPopup($cell, $cell.chest.html, 'Круто!', 'Закрыть окно', () => okClicked($cell));
}
