...
- `/set_group` — Установить группу школьнику (только из доступных ему групп)
...
### Группы и доступ
Группы задаются в Google Sheets. Учитель видит и меняет группы только в рамках своего `allowed_groups` и доступных групп ученика.
Если `allowed_groups` пустой, учитель считается имеющим доступ ко всем активным группам.
