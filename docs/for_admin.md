### Команды для админов
- `/broadcast` — Отправить сообщение в строчках 3+ всем юзерам из строчки 2. Вместо списка токенов во второй строчке можно указать `all_students`,
  `all_teachers`, `all_group:<group_id>` или код группы из `groups.broadcast_code` (например `all_novice`).
...
- `/update_groups` — Обновить группы из гугль-таблички
