"""Exact Phase-6F Student-reaction migration lifecycle."""
...
MIGRATION_ID = "0055.pwa_submission_review_student_reactions"
    ...
    with sqlite3.connect(database_path) as connection:
        return {
            ...
            for row in connection.execute(
                ...
                "AND name LIKE 'submission_review_student_reaction%'"
            )
        }


def test_student_reaction_migration_up_down_up_is_exact(tmp_path):
    database_path = tmp_path / "phase6-review-student-reactions.sqlite3"
    ...
    expected = {
        "submission_review_student_reactions",
        "submission_review_student_reaction_events",
    }
    ...
    with sqlite3.connect(database_path) as connection:
        state_columns = {
            str(row[1])
            for row in connection.execute(
                "PRAGMA table_info(submission_review_student_reactions)"
            )
        }
