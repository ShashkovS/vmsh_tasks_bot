...
PWA_E2E_ENV := VMSH_ANALYTICS_DB_FILENAME=.runtime/vmshpwa/e2e/analytics.sqlite3 VMSH_RUNTIME_PROFILE=pwa-e2e VMSH_INSTANCE=e2e VMSH_DB_FILENAME=db/vmshpwa_e2e.sqlite3 VMSH_MEDIA_ROOT=.runtime/vmshpwa/e2e VMSH_NATS_SERVER= VMSH_NATS_TOPIC_PREFIX=vmshpwa_e2e VMSH_PWA_PROTOTYPE=true
...
pwa-analytics:
	$(PWA_UV_ENV) $(PWA_HUMAN_ENV) uv run python -m vmshpwa.scripts.course_analytics

pwa-agent-analytics:
	$(PWA_UV_ENV) $(PWA_AGENT_ENV) uv run python -m vmshpwa.scripts.course_analytics
...
.PHONY: pwa-family-link-import-preview
pwa-family-link-import-preview:
	@test -n "$(PWA_FAMILY_LINK_IMPORT_DATABASE)" || (echo "Set PWA_FAMILY_LINK_IMPORT_DATABASE"; exit 2)
	@test -n "$(PWA_FAMILY_LINK_IMPORT_CSV)" || (echo "Set PWA_FAMILY_LINK_IMPORT_CSV"; exit 2)
	$(PWA_UV_ENV) uv run python -m vmshpwa.scripts.family_link_import --database "$(PWA_FAMILY_LINK_IMPORT_DATABASE)" --csv "$(PWA_FAMILY_LINK_IMPORT_CSV)" $(if $(PWA_FAMILY_LINK_IMPORT_REPORT),--report "$(PWA_FAMILY_LINK_IMPORT_REPORT)",)
...
pwa-two-worker-local-smoke:
	$(PWA_UV_ENV) VMSH_RUN_TWO_WORKER_SMOKE=1 uv run pytest -q -n0 pwa_tests/integration/test_two_worker_live.py
...
pwa-production-build:
	@test -n "$(PWA_RELEASE_ID)" || (echo "Set PWA_RELEASE_ID"; exit 2)
	@test -n "$(VITE_PUBLIC_MEDIA_ORIGIN)" || (echo "Set VITE_PUBLIC_MEDIA_ORIGIN"; exit 2)
	@test -n "$(VITE_SENTRY_DSN)" || (echo "Set VITE_SENTRY_DSN"; exit 2)
	@cd $(PWA_DIR) && CI=true \
		VMSH_FRONTEND_BUILD_PROFILE=production \
		VITE_ENABLE_MSW=false \
		VITE_PROTOTYPE=false \
		VITE_PUBLIC_MEDIA_ORIGIN="$(VITE_PUBLIC_MEDIA_ORIGIN)" \
		VITE_SENTRY_DSN="$(VITE_SENTRY_DSN)" \
		VITE_SENTRY_RELEASE="$(PWA_RELEASE_ID)" \
		pnpm build
...
pwa-e2e-redeploy:
	cd vmshpwa && CI=true pnpm e2e --mode redeploy
...
pwa-e2e-support:
	cd $(PWA_DIR) && CI=true pnpm e2e:support
...
pwa-e2e-news:
	cd $(PWA_DIR) && CI=true pnpm e2e:news
