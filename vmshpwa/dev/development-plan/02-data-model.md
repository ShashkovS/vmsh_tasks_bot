...
| ------------------------------------------------- | ----------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `users`                                           | Ученик/учитель/admin, Telegram, активная группа, token, online, grade, birthday, allowed groups | Сохранить primary domain identity; не переносить массово. Auth account ссылается на `users.id`. Добавить nullable opaque `public_id`: controlled activation/backfill назначает случайное стабильное значение, а отсутствие ID закрывает browser-доступ. `grade`/`birthday` остаются nullable источником classroom read model. Нормализовать `allowed_groups` позже без удаления legacy-поля. |
...
Browser-контракт различает три идентичности: внутренний `users.id`, внешний
`users.public_id`, выдаваемый как `userId`/`studentId`, и независимый
`auth_accounts.public_id`, выдаваемый как `accountId`. Их нельзя подменять друг
другом. Legacy-строки до активации могут иметь `users.public_id IS NULL`; API
для такой строки fail-closed, а production backfill присваивает
криптографически случайное непрогнозируемое значение без изменения integer FK.
