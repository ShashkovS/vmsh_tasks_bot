...
профиле Student/Family и в шапке Staff; выбор перезагружает страницу. Оболочка,
навигация, вход, стартовые и офлайн-экраны и ошибки входа переведены на
английский; русский остаётся по умолчанию. [План и пруфы](../development-plan/24-i18n.md).
...
макросах — ключ), каталоги `.po` у пакетов и приложений, `@vmsh/i18n`
(язык до первого рендера, cookie `vmsh-locale`, кешированные `Intl`), guard
сборки и `make pwa-i18n-extract` / `pwa-i18n-check`. Backend переводит
`PwaApiError` на язык cookie. Замеры в бюджете: +2.7–3.8 KB brotli, FCP ±12 мс.
[План и пруфы](24-i18n.md), [ADR 0004](../../../adr/0004-pwa-internationalization.md).

## Полная перепроверка тестовой задачи — 17 сентября 2026, реализовано локально

Миграция 0095 хранит полную актуальную attempt-проекцию и фильтрует устаревшие
автоматические `results` во всех общих read-models. Все PWA-ответы конкретной
задачи пересчитываются вне write-транзакции и применяются атомарно; copy-only
изменения не создают result, конфликт metadata даёт 409, повтор идемпотентен.
Student/Family/Staff получают realtime invalidation. [Контракт и трассировка](../../docs/test-attempt-recheck.md).

Локальные repository/HTTP/UI/schema/typecheck/build gates пройдены. Playwright
submission-сценарий на Python 3.14 пока блокируется до запуска приложения
совместимостью `aiogram`/Pydantic (`RecursionError` при импорте `aiogram.types`),
до выполнения самого браузерного теста.

## Вопросы с эмодзи — 16 сентября 2026, исправлено локально

Исправлен подсчёт длины превью: Unicode code points вместо UTF-16.
11 контрактных/клиентских тестов прошли, включая оба фильтра и пагинацию.
Требуется frontend-выпуск. [Отчёт](../../docs/support-unicode-excerpts.md).

## CSP рисунков — 16 сентября 2026, выпущено

S3 добавлен в connect-src nginx, проверка конфига ловит повторение ошибки.
27 тестов прошли. Root reload применён; выпуск `4f273aac29c9-20260916153458`
активен, SHA256 обоих SW изменился. Service-status ready, все runtime HTTP 200. [Диагностика](../../docs/media-csp.md).
...
проверки сохранены. 8 тестов прошли. Нужна установка root-owned скрипта перед
push, иначе повторится блокировка production. [Runbook](../../docs/smooth-redeploy.md).

## Python-шаблоны ответов в Safari — 16 сентября 2026, проверено локально

Устранена блокировка всех ответов при неподдерживаемом RegExp:
проверку формата выполняет сервер. Исправлено сравнение «МИ»/«ми» с сохранением
прежних совпадений похожих букв; новые проверки имеют версию v2.
21 unit, 24 browser (Chromium/WebKit/Firefox), 108 backend прошли.
TypeScript и целевой ESLint прошли. Production и прежние оценки не менялись. [Причина и контракт](../../docs/answer-pattern-compatibility.md).

## Уровни задач устного приёма — 16 сентября 2026, проверено

Production: 16 сентября, 10:04 UTC активирован `ff9f2fb6a496-20260916080639`.
Публичные HTML/JS проверены, все runtime — 200; [подробности](../oral-levels-report.md).

Переключение опубликованных уровней внутри одной сессии без изменения группы
школьника; независимые кнопки «Устные / Все». Реализация и проверки:
[отчёт](../oral-levels-report.md), [требования](../../docs/live-marking.md).
30 unit, 14 backend и 15 E2E прошли; Chromium/Firefox/WebKit, обе темы,
1440/390/320 px, offline, undo, WebSocket и очная регрессия. TypeScript,
целевой ESLint и сборки прошли. Снимки сохранены; production не менялся.
...
на неделю с исключением отдельных дней. Student: скрытые пустые/прошедшие окна
и компактный открытый/будущий приём. Общие окна связаны одной записью;
изменение и отмена применяются ко всем выбранным группам. Миграция 0094,
атомарный batch с идемпотентностью, сохранение черновика после reload.
Браузерный сценарий прошёл в Chromium, Firefox и WebKit; собраны три приложения.
Пройдены 15 backend/runner и 7 frontend unit-тестов. 15 сентября владелец
разрешил commit и push; завершение production-деплоя ещё не подтверждено.
[Требования и файлы](../../docs/oral-window-weekly-drafts.md).

## Явные переходы устных оценок — 14 сентября 2026

Реализованы `было → станет`, красное неподтверждённое состояние, локальное
снятие собственных оценок текущего приёма и исправления undo (полный цикл,
in-flight, потерянный ответ, durable undo). Предыдущие results и чужие оценки
не удаляются. [Решение и код](../../docs/live-marking.md#явные-переходы-и-локальный-сброс--2026-09-14).
Пройдены 23 backend/runner и 28 frontend unit, typecheck, целевой lint, 10 browser stories.
Сквозные проверки: 12 passed в Chromium, WebKit и Firefox, включая offline
сразу после undo receipt. Собраны все три приложения; изменения не выпущены.
[Отчёт](../../../pwa_tests/reports/live-marking-toggle.md).

## Фотографии в ответах Staff — 14 сентября 2026, реализовано локально

Учитель и администратор прикрепляют фото в приватной переписке по задаче,
школьник видит ответ с фотографиями. Общий composer сохраняет бинарный черновик
и загрузки при ошибке, поддерживает отправку картинки с подписью «Фотография»
без набора текста. Чужие загрузки и доступ вне назначений запрещены;
идемпотентность и текстовые клиенты сохранены. Новая миграция не требуется.
См. [описание и файлы](../../docs/question-photos.md).

Проверено: HTTP-тесты фото (3), регрессия support API/repository (16),
frontend unit (7), typecheck Staff/Student/app-shell/product и сборки Staff/Student.
Браузерная проверка общего photo picker прошла в Chromium (320/390/800 px).
Новые lint-ошибки устранены; существующее предупреждение Fast Refresh в
`staff-support-pages.tsx` остаётся. Изменения ещё не выпущены.
...
последовательный Argon2 для большой пачки пересёк 120-секундный timeout.
Student/Family apply разделён клиентом на порции по 20 с прогрессом; backend
изолирует каждую строку savepoint. Валидные строки сохраняются, пропущенные
показываются с именем, логином и причиной; ошибки остаются в TSV для исправления.
Family-дубли определяются по занятому login или email, считаются отдельно и не
получают новый login suffix. Вторая ошибочно созданная пачка: 1138 аккаунтов
архивированы, 1197 связей отозваны; первая пачка активна. Перед исправлением
создана и проверена восстановимая SQLite-копия, операция записана в auth/audit.
Проверки: 7 HTTP integration и 7 frontend unit, Ruff/ESLint, Staff typecheck
и production-сборка прошли.
См. `05-phase-1-auth.md` и `account-provisioning-batch.ts`.

## Серийная проверка: лента и перенос — 10 сентября 2026, готово к выпуску

Принят [план](../../docs/serial-review-feed.md): накопление карточек, собственная
история по 20, условие, перенос/клон целой посылки с безопасным обновлением очередей.
Реализованы лента, маршрутизация, CAS/idempotency и миграция 0087.
89 backend- и 16 frontend-тестов прошли. Финальные шесть E2E-сценариев
прошли в Chromium/WebKit/Firefox; typecheck, Ruff/ESLint и сборки прошли.
Выпуск через совместимую миграцию 0087 и deploy-hook; после push проверяются
revision receipt, сервисы и свежие ошибки Sentry.

## Отправка выбранного ответа — 9 сентября 2026

Исправлен перехват кнопки общей очередью: отправка привязана к ID записи,
повтор восстанавливает актуальное состояние из IndexedDB. 18 тестов,
Student typecheck, ESLint и сборки всех PWA прошли. Браузерный E2E не запускался.
См. `docs/support-problem-context.md`.

## Исправление лимитов и совместимости вопросов — 9 сентября 2026

Лимиты не повторяются автоматически, сообщение без технического кода.
Support extensions только по opt-in заголовку. 56 backend / 14 frontend тестов,
Student/Staff typecheck, ESLint и сборки всех PWA прошли. Повтор старых
429-отказов проверен. Реализация:
[описание](../../docs/support-problem-context.md).

## Контекст задачи в вопросах — 9 сентября 2026

Полный номер и название в Staff, раскрываемое опубликованное условие в карточке.
Реализация и проверки: [support-problem-context.md](../../docs/support-problem-context.md).

## История проверок — 9 сентября 2026, готово к выпуску

Backend/read model и Staff UI реализованы; **73 backend + 23 frontend теста** прошли.
Полный сценарий истории прошёл в Chromium, WebKit и Firefox (**3 passed**).
Ruff, ESLint, Staff TypeScript и сборки всех PWA прошли.
Миграция 0086, новых серверных настроек нет.

Реализован [принятый план](../../docs/review-history.md): свои/все завершённые
проверки, фильтры, append-only перепроверка с аннотациями, защита гонок и возврат.

## Учительский режим тестирования — 9 сентября 2026, готово к выпуску

- Полный цикл сдачи без участия в общей статистике; отдельные представления
  Staff для учителей и администраторов. Контракт: [staff-testing.md](../../docs/staff-testing.md).
- Проверены границы тестовой идентичности, актуальные права и повторное
  использование настоящего Student UI, а не имитация проверки ответов.
- Новые HTTP 6/6, UI 3/3; schema/new-mode 27/27, typecheck Staff/Student,
  ESLint, Ruff и production builds PASS. Полные наборы: backend 1852 PASS,
  6 SKIP / 5 прежних failures; frontend 699 PASS / 3 прежних failures.
  Все восемь падений воспроизведены отдельно на исходном HEAD; список — в doc.
...
- На изолированной полной схеме воспроизведена деградация даже `SELECT 1`
  при параллельном открытии соединений. Добавлены отдельные async-лимиты
  чтений/записей в существующий PwaConnectionFactory, без кеша данных.
- Контракт, измерения и оставшиеся production-проверки:
  [sqlite-admission-performance.md](../../docs/sqlite-admission-performance.md).
- 136 тестов прошли: допуск/отмена/rollback, trace, auth HTTP/repository,
  сдача тестовых ответов и migration/runtime lifecycle. Ruff и diff-check
  прошли. Production-эффект пока не измерен; миграций/новых настроек нет.

## Имя в шапке кабинета — 9 сентября 2026

- AppShell показывает displayName текущей сессии вместо общей подписи роли;
  подключено в Student/Family/Staff root routes, включая мобильную и компактную шапку.
- Без дополнительных API-запросов. App-shell stories проверяют видимость имени
  во всех трёх аудиториях; до появления имени сохраняется прежняя подпись.
- Проверки: 4 браузерных теста, typecheck всех трёх приложений, ESLint прошли.

## Диагностика ошибок отправки — 9 сентября 2026

- Разделены серверные, транспортные, auth и клиентские ошибки вместо «нет сети».
- Sentry: безопасные ID и requestId; общий перехват ошибок query/mutation.
- [Контракт и проверки](../../docs/submission-error-diagnostics.md): 35 unit-тестов,
  сборки Student/Family/Staff прошли. На production не развёрнуто.

## Трейсинг задержек — 8 сентября 2026

- Добавлены безопасные агрегированные timings запросов от 200 мс и loop lag.
- [Реализация и инструкция сбора](../../docs/request-tracing.md).
- Проверки: 34 теста (trace, SQLite concurrency, HTTP auth, Prometheus), Ruff
  и diff-check прошли. Production-сбор и установление причины — после выпуска.

## Поиск школьника в статистике — 7 сентября 2026

- Реализован локальный нечёткий фильтр вместо полного выпадающего списка;
  загрузка графика только после выбора, ID по-прежнему хранится в URL.
...
## Устойчивый вход школьника — 7 сентября 2026

- `models/pwa/auth.py`, `apps/pwa_api/auth_service.py` и пути выдачи Student
  аккаунтов используют отдельную нормализацию школьных учётных данных:
  Unicode-пробелы удаляются полностью, а регистр приводится к `casefold`.
  Семантика паролей Family и Staff не меняется. Проверка покрывает пробелы,
  табуляции, переводы строк и неразрывный пробел для логина и токена.
- `/student/login?login=...` передаёт логин в форму. При активной сессии форма
  не показывается: существующий auth-hook сначала завершает проверку и
  перенаправляет школьника в приложение.
- Backend-focused tests **121 PASS**, Ruff и Student strict TypeScript —
  **PASS**. Полный frontend unit прогон подтверждает новый login-page тест;
  два чужих падения остаются в `packages/app-shell/src/notification-client.test.ts`
  (тест ждёт 9 категорий вместо фактических 10).

## Компактная продуктовая аналитика — в работе, 7 сентября 2026

- Начата отдельная `analytics.sqlite3`: она будет жить в WAL отдельно от
  учебной БД, принимать только компактные best-effort события PWA и не станет
  зависимостью сдачи задач или публикации. Реализация: `main.py`,
  `db_methods/pwa/product_analytics.py`, `apps/pwa_api/product_analytics_routes.py`
  и общий трекер `packages/app-shell`.

## Компактная продуктовая аналитика — 7 сентября 2026

- Добавлены независимые `analytics.sqlite3` и lifecycle в `main.py`: схема
  имеет версию, WAL и индексы, startup и ежедневная задача удаляют записи
  старше начала текущего учебного года (10 августа, Europe/Moscow). Отказ
  аналитики fail-open: ни API, ни учебная БД не становятся недоступными.
- Все защищённые оболочки Student, Family и Staff автоматически отправляют
  `page.view` без URL-параметров; общая очередь ограничена 20 событиями,
  отправляется `fetch(..., keepalive)` и очищается при уходе со страницы.
  Центральные клиенты уже отмечают успешную сдачу теста/письменного решения,
  фото и создание/ответ в вопросе. В событие входят только компактные
  разрешённые поля, включая выбранные точные viewport и DPR.
- Staff получил capability `product-analytics.read`, только у глобального
  администратора, новый экран `/staff/analytics`, сводку, фильтры и ленту.
...
посылке, один подготовленный следующий захват, горячие клавиши отправки и
исправление предыдущей работы. Существующие lease/correction API остаются
авторитетными для прав и конфликтов. Проверено: 45 серверных тестов (включая
конкурентный захват и correction), 16 frontend unit, 8 браузерных Storybook
...
## Вопросы организаторам — 11 сентября 2026, реализовано локально

Принят [план](../../docs/organizer-questions.md): Student/Family обращения,
приватные ответы администраторов, фото, уведомления и отдельная очередь.
Схема 0088, API, бинарные черновики, Student/Family и очередь Staff реализованы.
Проверено: backend 14 + migration rehearsal 1, unit 5, E2E 9 без повторов во всех
трёх движках; mobile 320/390, desktop, 200%, тёмная тема, фото, уведомления,
WebSocket и восстановление после offline/reload. TypeScript, ESLint, Ruff и
production build прошли. [Отчёт и снимки](../../../pwa_tests/reports/organizer-questions/README.md).
Выпуск требует миграции 0088. Владелец разрешил commit и push 11 сентября 2026.
Развёртывание не выполнялось; визуальное принятие владельцем остаётся открытым.

## Названия задач — 11 сентября 2026, реализовано локально

[Требование владельца](../../docs/task-titles.md) выполнено: названия видны
в листках Student/Family, очной и Zoom-сетке и результатах школьника.
Используются метаданные своей версии и названия самостоятельных пунктов;
длинные строки переносятся. В Zoom на тестовом 390 px помещаются 24 задачи
с названиями и двумя независимыми кнопками по 44 px.

Проверено: backend **5 passed**, unit **34 passed**, затронутые Storybook
сценарии **5 passed**, E2E **18 passed** Chromium/WebKit/Firefox и после
финального уменьшения отступов Zoom ещё **6 passed**. TypeScript приложений,
ESLint, Ruff, Prettier и production build прошли. Снимки просмотрены.
[Отчёт, снимки и известное ограничение старого Storybook-теста](../../../pwa_tests/reports/task-titles/README.md).
Миграция и перекомпиляция листков не нужны. Владелец разрешил commit и push
11 сентября 2026. Серверное развёртывание не выполнялось.

## Кнопка обновления PWA — 11 сентября 2026, исправлено локально

Общий `usePwaUpdateActivation` восстанавливает нажатие на плашку после активации
worker в другой вкладке. Student/Family показывают «Обновляем…», таймаут и
ошибку с повтором; сохраняются текущий URL и безопасные моменты перезагрузки.
Проверено: **8 unit**, **12 E2E** без повторов Chromium/WebKit/Firefox,
TypeScript приложений/app-shell/tooling, ESLint, Prettier и production build.
[Решение](../../docs/runtime-isolation.md#восстановление-кнопки-обновления--11-сентября-2026),
[отчёт](../../../pwa_tests/reports/pwa-update-activation.md).
Владелец разрешил commit и push 11 сентября 2026. Серверное развёртывание не выполнялось.

## Кавычки в названиях задач — 11 сентября 2026, готово локально

По уточнению владельца `SemanticMathDocument` показывает `Задача 12 «Лабиринт»`
и `а) «Разрезалка А»`. Общий рендер Student/Family/Staff и соответствующая
...
В обоих SW переход выполняет общий openPushNotification через прямой openWindow,
...
## Исправление каталога материалов для разбора — 2026-09-13

Контракт допускает занятие 0 («Знакомство»): ранее оно отклоняло весь каталог.
...
## Приоритет автоматического зачёта теста — 2026-09-13

Исправлена застревавшая ручная оценка тестовой задачи: более новый успешный
автоматический тест снимает старую live-фиксацию, неверная попытка её не снимает,
а более поздняя ручная оценка снова имеет приоритет. Миграция
`0091.pwa_test_result_live_mark_precedence` обновляет insert/update-триггеры и
ремонтирует уже затронутые ячейки по порядку неизменяемого results ledger.
Контракт: [live-marking.md](../../docs/live-marking.md). Покрыты migration
up/down/up, ремонт данных, перепроверка теста и HTTP-сценарий от ручного минуса
до статуса `accepted` в Student. Проверено: 39 migration/live/schema и 16
зависимых тестов архива результатов, статистики и legacy-порогов; Ruff,
schema inventory и `git diff --check` прошли.

## Понятная очередь проверки — 2026-09-13, реализовано локально

Сводка считает уникальные работы; карточки явно показывают ожидание, доступность
и занятость коллегами. Добавлены фильтры курса/группы и сортировка по числу работ,
URL сохраняется при возврате из одиночной/серийной проверки. API и leases прежние.

Проверки: 22 unit-теста, 2 browser stories, 9 E2E с реальным изолированным backend
в Chromium/WebKit/Firefox. После визуальных поправок все 3 E2E очереди повторно
прошли: 320/390 px, обе темы, клавиатура и CSS zoom 200%. Исправлена нативная
...
## Поиск школьников по ФИ и фамилии — 2026-09-13, реализовано локально

Поиск в Zoom и при переносе в аудиторию больше не учитывает отчество. Добавлен
переключатель «Искать только по фамилии» с прежней нормализацией, поиском по
фрагменту и допустимыми опечатками. Directory API передаёт фамилию отдельным
полем, поэтому составные фамилии не разбираются из отображаемого имени.
Контракт: [live-marking.md](../../docs/live-marking.md).

Проверено: 11 целевых frontend unit-тестов, 12 backend integration-тестов и
новый E2E в Chromium/WebKit/Firefox; Staff/Contracts typecheck, ESLint, Ruff и
production build прошли. Полный `e2e:oral` по-прежнему содержит прежние падения
в live-marking/oral-admission: старые ожидания короткого номера `Задача 1` не
соответствуют текущему полному номеру вида `Задача 931н.1`.
...
## Временный API для a22/a23 — 2026-09-14, выпущено

Token-only bridge получил `lesson-results` с активным roster курса, задачами,
последней активностью и полной матрицей завершённого занятия. `cur_les` в
`a22`/`a23` явно задаёт номер обрабатываемого занятия независимо от календаря;
`event_id` отдельно выбирает опорный курс. Во время занятия 2 для рассылки
занятия 1 стоят `cur_les = 1` и `event_id = "ipe-1"`. Итоговый raw verdict
используется в персональных письмах, score с test-attempt penalty — для
`решено/всего` на старом сайте. Уровень выбирает общий алгоритм course analytics;
пустые legacy synonyms не объединяются.

Фактические `z_portal_print.py`, `a22_create_mails.py` и
`a23_mark_compl_in_html.py` в папке 2026–2027 переведены с локальной SQLite на
атомарный `portal-after-lesson.json`; прежнее значение 35 заменено явными
`cur_les = 1` и `event_id = "ipe-1"` в каждом скрипте. FTP-публикация при
проверке не запускается. Контракт:
`docs/printing/legacy-api.md`.

Коммиты `0580ffae` и `72330f03` отправлены в `vmshpwa`; webhook доставил API.
Production-smoke для `cur_les = 1`, `event_id = "ipe-1"` вернул 1266 учеников,
42 задачи и 7856 ячеек. Среди них 1812 пустых, 516 нулевых, 13 половинных и
...
Все 12 фигур представлены нативными TikZ-контурами с прежними командами;
PNG и специальные шрифты при компиляции не нужны. 99 тестов прошли, включая
реальный PDF → SVG, отсутствие растра и масштабирование. Примеры просмотрены.
[Требования](../../docs/tikz-chess.md), [отчёт](../tikz-chess-report.md).

## Возвращён вход в окна устного приёма — 2026-09-14

В списке занятий и шапке конкретного занятия добавлена ссылка «Окна устного
приёма» с выбранным groupLesson и tab=windows. Видимость совпадает с формой:
admin с oral.manage. Реализация: `staff-lessons-page.tsx`, `content-page.tsx`;
маршрут и API прежние. [Инструкция](../../docs/classroom-and-oral-workflow.md).
Staff typecheck и production build прошли. ESLint сообщает три прежних ошибки
в content-page.tsx, подтверждённые на HEAD. Старый UploadPreviewPublish проходит
новую ссылку, но падает на существующем ожидании текста preview. Отдельная
story OralWindowsNavigation прошла в Chromium: адрес занятия, видимость и
...
преобразуются в векторные вставки при подготовке TikZ; обычные рисунки
...
Сохранение вынесенных рисунков и разделение пунктов, черновой редактор с конфликтами,
неизменяемые публикации, повторная обработка READY-исходника реализованы.
PDF исключён из объёма по указанию пользователя. Проверены повторная обработка,
откат и отложенная активация с отличающимися черновиками; Telegram-адаптер проверяется
без отправки реальным адресатам. 178 backend-тестов и 9 браузерных сценариев
прошли; Chromium/Firefox/WebKit, 1280/320/390 px, обе темы, клавиатура, print и
фактические PNG/ZIP. Staff/tools TypeScript, целевые линтеры и unit-тест проходят.
Снимки сохранены. Пользовательская приёмка и выпуск остаются за владельцем.
[Контракт](../../docs/figure-layout.md), [отчёт](../figure-layout-report.md).

## Компактное раскрытое решение — 2026-09-15

Убраны дублирующая служебная подпись «Решение» и верхний отступ первого
