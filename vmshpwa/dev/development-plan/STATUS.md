...
последней активностью и полной матрицей текущего занятия. Итоговый raw verdict
...
атомарный `portal-after-lesson.json`; жёсткий прошлогодний `cur_les = 35` удалён.
FTP-публикация при проверке не запускается. Контракт:
