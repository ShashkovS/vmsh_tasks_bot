...
| `a01_school_to_conduit.py`                         | Выгрузка текущего mode/group/strength и посещаемости из SQLite в ведомость через clipboard  | Staff statistics/classroom export; до cutover — versioned compatibility snapshot  | Этапы 7/9/10 и последующий export cutover           |
| `a02_welcome_aud.py`                               | Генерация Telegram-команд рассылки аудиторий по `IDd`                                       | notification/delivery workflow; в v1 — legacy adapter от confirmed plan snapshot  | Этап 7: bridge parity; полный cutover позже         |
...
| `a11_spis_from_xls.py`                             | Excel-аудитории, barcode/кондуиты, списки на двери/этаж и PDF                               | Staff classroom plan + будущий print service; в v1 legacy bridge                  | Этап 7 bridge и отдельная фаза print                |
...
Apply принимает hash именно просмотренного входа и dry-run report, выполняется транзакционно и пишет audit/import report. Повтор с тем же hash либо безопасно возвращает прежний receipt, либо явно останавливается как уже применённый. Сформированные комнаты не получают capacity/weight из исторических counts. После cutover catalog/layout/plan редактируются только Staff API. По рекомендуемому варианту `CLASSROOM-01` confirmed plan создаёт узкий hash/version-stamped compatibility snapshot с колонками, нужными `a02`/`a11`–`a14`; stale snapshot отклоняется. Полноценный print/delivery workflow всё равно переносится отдельно и только тогда legacy scripts выключаются.
