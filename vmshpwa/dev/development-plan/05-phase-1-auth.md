...
- `apps/pwa_api/account_batch_routes.py` реализует отдельные admin-only
  ...
  повторно проверяет просмотренный input. Каждая строка защищена SQLite
  savepoint: локальный конфликт пропускает только эту строку, а не откатывает
  уже готовые аккаунты пакета;
...
### Большие пакеты и отчёт ошибок — 12 сентября 2026

Production access-log подтвердил причину неясной ошибки большого Family-пакета:
preview завершился с `200`, а единый apply получил `504` ровно на границе
120-секундного Gunicorn/nginx timeout. До write-транзакции endpoint
последовательно вычислял Argon2 для всех паролей.

Staff теперь применяет Student/Family строки независимыми порциями по 20.
Каждая порция повторно получает preview непосредственно перед apply, поэтому
дорогой хеш не держит один HTTP-запрос минуты, а успешно завершённые порции не
откатываются из-за следующей. Кнопка показывает прогресс. Итог объединяется по
исходным номерам строк; все пропуски показаны отдельной таблицей с именем,
логином и причиной. В поле TSV автоматически остаются только пропущенные строки
для исправления и повторной проверки. При наличии ошибок preview показывает все
ошибочные строки, а не только первые 100 строк исходного пакета.

Для Family действует бизнес-ключ владельца: нормализованный login и каждый email
должны быть уникальны среди всех предыдущих загрузок и текущей таблицы. Совпадение
любого из них помечается дублем; новый login с суффиксом `-NN` не создаётся.
Preview и итог показывают отдельное число дублей и точную причину по строкам.
Дубли автоматически удаляются из локального TSV, а строки с настоящими ошибками
остаются для исправления. После каждой подтверждённой порции созданные строки
также сразу удаляются; перезагрузка продолжает только ещё не подтверждённую часть.

Read-only audit инцидента показал две полностью завершённые одинаковые Family
пачки: по 1138 строк в `09:58:34Z` и `10:04:06Z`; во второй все 1138 строк
совпали с первой по имени и provisioning password. После отдельного подтверждения
владельца 12 сентября вторая пачка архивирована без физического удаления:
1138 аккаунтов переведены в `archived`, 1197 связей с детьми отозваны, первая
пачка осталась активной. До операции ни один дубль не логинился и не имел сессий.
Auth/audit-события сохранены под request
`repair-family-duplicates-20260912T111524Z`; восстановимая копия:
`/web/vmsh_tasks_bot/backups/vmsh-before-family-dedupe-20260912T111524Z.sqlite3`.

Реализация: `apps/staff/src/account-provisioning-batch.ts`,
`account-provisioning-page.tsx`, `apps/pwa_api/account_batch_routes.py`.
Пруф: unit-тесты chunking/интерфейса/TSV и HTTP-интеграция с искусственным
row-local `IntegrityError`, проверяющая `created / skipped / created` и отсутствие
частично записанного проблемного аккаунта. Повторы login/email из прежней загрузки
и внутри одной таблицы проверяются отдельными HTTP assertions.
