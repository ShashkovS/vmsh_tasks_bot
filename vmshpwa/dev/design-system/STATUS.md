...
Исправлены несвязанные с переименованием роли lint/unit/Storybook сбои:
семантика фильтров аналитики, избыточная синхронизация состояния и устаревшие
fixtures/play-ожидания. Пройдены форматирование, lint, typecheck, i18n-check,
все PWA builds, 872 frontend- и 2053 Python-проверки (6 skipped), а также
313 Storybook play-проверок из 63 файлов. Визуальные подписи и техническая
граница `/family` сохраняются. Browser-истории выполняются последовательно,
чтобы нагрузка соседних вкладок не искажала 50-мс interaction budget
live-marking.

## Терминология родителя — 19 сентября 2026, реализовано локально

Во всех экранных и PWA-подписях Family/landing/Staff и общих компонентах
«семья» заменена на «родитель» с согласованными падежами. Сохранены технические
границы `/family` и `family`; данные отображаемых аккаунтов не переименовывались.
Пройдены i18n-check, typecheck, сборки и 75 runtime E2E в трёх браузерах;
несвязанные с copy общие lint/unit/Storybook сбои исправлены в записи выше,
таймаут первого news E2E сценария задокументирован в development-plan status.

## Интернационализация, P0 — 18 сентября 2026, реализовано локально

Storybook получил глобал «Язык интерфейса» (Русский / English) и оборачивает
истории в `LocaleProvider`; по умолчанию истории и их play-тесты остаются на
русском. Экран недоступного каталога — статический двуязычный `renderCatalogFailure`.
Переключатель языка в кабинетах — P1. [План](../development-plan/24-i18n.md).

## Полная перепроверка тестовой задачи — 17 сентября 2026, реализовано локально

В подтверждённых metadata тестовые задачи с ответами показывают число попыток,
актуальность проекции и переход на массовую перепроверку. Preview называет задачу,
правильный ответ, школьников, изменения verdict/формата/реплик; запуск требует
явного подтверждения, receipt показывает фактическое применение. Student и Family
обновляются realtime без искусственных сообщений. [Контракт](../../docs/test-attempt-recheck.md).
...
## Спокойный редеплой — 15 сентября 2026, проверка

Phase 5: общий runtime показывает ожидание вместо ошибки безопасности при
временном сбое. Открытый экран не заменяется: компактная плашка над кабинетом,
нейтральные сообщения, отдельное пояснение после 60 секунд. Реализация и
границы выпуска: [smooth-redeploy.md](../../docs/smooth-redeploy.md).
Stories: `App shell/Service recovery` (обновление, долгое ожидание, соединение).
Chromium/WebKit/Firefox: все 15 E2E прошли — холодный старт, ожидание более минуты,
форма через 20 секунд, фото и оценка с потерянным ответом. Три Storybook/axe
состояния прошли; снимок Staff просмотрен, snapshots не менялись. Production и ручное
принятие владельцем остаются открыты.

## Короткие номера пунктов — 13 сентября 2026

В общем листке заголовок задачи остаётся полным (`Задача 1э.2.`), а пункт
получает компактную подпись (`2а)`). Названия и полные контрактные ссылки
сохранены; см. [`task-titles.md`](../../docs/task-titles.md).
Проверены общий renderer и Staff preview, включая legacy-fallback.

## Импорт аккаунтов: частичный результат — 12 сентября 2026

Большие Student/Family пакеты показывают числовой прогресс. При ошибках preview
выводит все проблемные строки; после apply таблица «Пропущены» показывает
исходный номер, имя/логин и понятную причину. Family-дубли login/email считаются
отдельно и удаляются из редактора; в нём остаются только настоящие ошибки.
Покрыто interaction-тестом `account-provisioning-page.test.tsx`.

## Серийная проверка — 10 сентября 2026, проверено

Лента завершённых карточек без редакторов, inline-перепроверка с сохранением
текущего черновика, отложенные фотографии, отдельные панели навигации и серии.
Действия переноса/клона размещены непосредственно под выбранной посылкой.
См. [решение и проверки](../../docs/serial-review-feed.md).
Финальный прогон: оба сценария зелёные во всех трёх движках (6 E2E),
16 unit-тестов, typecheck, ESLint и сборки приложений прошли.

## Лимиты ответов — 9 сентября 2026

Часовой/дневной лимит показывается как понятная причина отказа без строки
HTTP/request ID; outbox не показывает его как ожидающую сеть отправку.
Проверено тестами observability/outbox, сборки прошли. См.
...
с настоящим Student UI через собственную тестовую личность. Admin сохраняет
редактор и получает `/staff/testing`. В Student виден режим и возврат в Staff;
перед заменой Student-сессии есть предупреждение. «Устные» скрыты в навигации.
Требования и proof: [staff-testing.md](../../docs/staff-testing.md).
UI interaction tests проходят; visual snapshots не менялись, owner review открыт.

## Поиск школьника в статистике — 7 сентября 2026

- Вместо select — нечёткий поиск имени, максимум 20 результатов и сброс выбора.
- Переиспользован поиск каталога; 11 unit-тестов, включая каталог из 2000 имён.
- Компонент: `apps/staff/src/statistics-student-search.tsx`.
...
- Проверены focused parser/editor contracts, Staff typecheck, lint и production build. Повторный news E2E ожидает доступный agent NATS: текущий запуск остановился на startup с `NoServersError` до Playwright. До acceptance нужны owner visual/a11y review wide/mobile layout; CodeMirror намеренно остаётся отдельным lazy Staff chunk.
...
Очередь Staff группируется по задачам. `ThreePaneReview` освобождает левую
колонку; `ReviewFeedbackForm` получает отправку Cmd/Ctrl+Enter. Предзагруженная
форма не принимает хоткеи. См. [решения режима](../../docs/serial-review.md).
Пройдены 8 браузерных сценариев Product/Review, включая Ctrl+Enter из поля
комментария; unit-тесты проверяют отключённую фоновую форму и подтверждение
незачёта. Серийный переход сохраняет смонтированную следующую работу.
...
Реализован [контракт](../../docs/worksheet-print.md): Student-only print CSS,
условия и открытые учебные материалы; ответы, фото работ и переписка скрыты.
Chromium/WebKit/Firefox, Storybook и PDF пройдены. Устранены лишние страницы
...
Student и Family worksheet получили shared Rich Markdown layout с независимыми
позициями до/после задач. Staff редактор показывает черновую, опубликованную и
ожидающую версии отдельно. Video placeholder загружает безопасно нормализованный
iframe только по действию читателя; print выводит ссылку вместо player.

23 сентября production release `b956cb1a2006` был активирован после migration
и backup; public CSP разрешает только YouTube и VK Video frames. YouTube embed
ответил с production Referer, supplied VK iframe требует VK Login для
анонимного просмотра. Нужен только Staff-авторский smoke в выделенном тестовом
занятии, чтобы зафиксировать реальное поведение provider-плееров в странице
урока.
...
Уточнение компоновки 23 сентября: шапка занятия, счётчик задач и кнопки
возвращены внутрь карточки задач. Материалы остаются отдельными карточками
с полями 16–28 px; video placeholder — компактная строка на всю ширину,
с названием провайдера и кнопкой загрузки. Между видео явный gap 16 px.
Проверены Student typecheck, renderer test, ESLint/Stylelint и сборка;
компоновка материалов просмотрена в Storybook через браузер.
