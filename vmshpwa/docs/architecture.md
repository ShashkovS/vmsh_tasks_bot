...
Telegram bot token принадлежит runtime credential profile, а channel destination не зашит в него: course/group `telegram_bindings` хранят verified canonical Bot API chat ID в SQLite. Content/news adapter разрешает effective binding по owner/purpose, сохраняет реально использованные binding/chat/message IDs и никогда не отправляет в глобальный fallback. Персональная classroom delivery разрешает Student bot-dialogue только server-side и не отдаёт destination browser-клиенту. Live test profile использует `@vmsh179devbot` и двухшаговый owner-only local test binding до появления production schema; hermetic unit/E2E остаются на RecordingBot.
...
Доменный код использует интерфейс `put/get/delete`. Unit/agent/E2E adapter пишет только внутрь выделенного media root и запрещает absolute/path traversal. Opt-in local integration и production используют S3-compatible bucket через `aioboto3`; browser upload всегда проксируется aiohttp, presigned upload не является частью контракта. Метаданные, связи, attachment revisions и audit остаются в SQLite. Реализованные profile, key, atomicity, redaction и disposable-smoke правила собраны в [object-storage contract](object-storage.md).

PWA storage config содержит явные `s3_url`, `s3_region`, `s3_bucket_name`, `s3_access_key`, `s3_secret_key` без production provider default: target — Hetzner. Для ручной local/test S3-интеграции allowlisted поля читаются из Beget test config и сверяются с pinned endpoint/bucket fingerprint, для production — из отдельного `creds_prod` config. Agent и обычный E2E не читают эти файлы и используют filesystem. Legacy default Beget в `helpers.config` сохраняется только для старых путей. Неполный S3 tuple завершается config error до client creation; access/secret key и object URL не входят в config representation, logs, Sentry, health или client contracts.
