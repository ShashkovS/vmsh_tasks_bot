...
`group_lesson` также владеет максимум двумя независимыми Rich Markdown
блоками (`before` и `after`). Они не являются задачами и могут быть доступны
раньше условий; подробный контракт и реализации описаны в
[lesson-blocks.md](lesson-blocks.md).
