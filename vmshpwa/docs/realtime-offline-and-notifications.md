...
На iOS потеря IndexedDB после долгого отсутствия допустима: авторитетные данные находятся на сервере. Student/Family мягко предлагают установку PWA, но отказ не блокирует работу.
