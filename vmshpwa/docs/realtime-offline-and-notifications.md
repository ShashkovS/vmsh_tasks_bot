...
Web Push показывает системное уведомление при каждом полученном push (включая foreground, согласно userVisibleOnly; см. [активацию уведомлений](notification-activation.md)) и делится на категории: учебный цикл, результат проверки, новый комментарий, новости, организационные сообщения. Пользователь отдельно разрешает категории; отказ браузера не блокирует кабинет. Notification click ведёт на устойчивый deep link внутри audience scope.
