...
- `cor_ans_checker` — optional trusted-admin Python checker для нестандартной проверки. В v1 сохраняется измеренная совместимость с текущим restricted-globals `exec`, но каждая ревизия, diff, ошибка выполнения и recheck аудируются. Это не teacher-интерфейс и не production sandbox для произвольного кода.
