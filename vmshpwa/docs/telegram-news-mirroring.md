...
Запланированные вручную в Telegram UI сообщения до фактической публикации остаются отдельным изменяемым внешним состоянием. Текущий bot/PWA ingest не считает эту pending-очередь своим источником истины и видит сообщение только после публикации. Перед включением Staff scheduler для конкретного destination и временного окна администратор обязан просмотреть очередь в Telegram UI и для каждой записи явно выбрать одно действие: оставить публикацию под управлением Telegram, отменить её и создать эквивалентную Staff-публикацию либо отменить как устаревшую. Сверка использует destination, плановое время, принятую content revision и media manifest; совпадение одного текста не является безопасной дедупликацией. Cutover закрывается reconciliation-отчётом без пропущенных и двойных публикаций.
...
Перед публикацией показываются два preview: PWA card/detail и Telegram. Для новых структурированных материалов Telegram adapter использует Bot API 10.2+ `sendRichMessage` и отдельный allowlisted Rich Message HTML с headings, paragraphs, emphasis/mark/sub/sup/spoiler, links, lists, quotes, code, details, tables, media и `<tg-math>`/`<tg-math-block>`. Новые условия задач отправляются полноценным текстом, не скриншотом; SVG/рисунки остаются media. Это не означает, что тот же markup можно передать в legacy `sendMessage(parse_mode=HTML)`. Renderer проверяет зафиксированные limits 10.2: 32 768 UTF-8 characters, 500 blocks, 16 уровней вложенности, 50 media и 20 table columns. Исторические fixtures берутся из `_external_pipelines/ChatExport_2026-07-25`.
...
Баннер имеет время начала и окончания; обычная публикация остаётся доступной без автоматического срока удаления. Group-scoped Staff announcement с Markdown, расписанием и выбранной аудиторией создаёт account-scoped PWA/Web Push событие в момент начала показа; правка до срока заменяет ожидающее событие, а после срока не посылает дубликат. Произвольные независимые от группы кампании, Telegram delivery и общий delivery dashboard относятся ко второй фазе. Существующая специальная административная группа остаётся будущей допустимой целью; исторических рассылок такого типа пока нет.
