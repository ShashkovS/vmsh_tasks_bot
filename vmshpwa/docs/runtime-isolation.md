...
- язык интерфейса устройства — осознанное исключение из namespace: cookie
  `vmsh-locale` (`Path=/`) общая для трёх аудиторий одного origin, потому что
  нужна экранам до входа и серверу при handshake. Она не содержит данных
  пользователя; авторитетна настройка аккаунта ([i18n](i18n.md)).
