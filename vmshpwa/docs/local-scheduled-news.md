...
Текущая модель получателей (группа, аудитория и очность) описана отдельно в
[`communication-targeting.md`](communication-targeting.md).
...
- Пока исходный `published_at` ещё не наступил, admin может изменить текст,
  время и фильтры получателей. Каждое содержательное изменение создаёт новую
...
- Опубликованную local PWA news admin может исправить. Правка создаёт новую
  ...
  `published_at`, получатели и порядок ленты при этом неизменны.
