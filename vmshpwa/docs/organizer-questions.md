...
## Оформление точек входа — 11 сентября 2026

Family `family-home-page.tsx`: основная кнопка с иконкой, отступом от занятий и полной шириной на телефоне. Staff `routes/questions.tsx`: общая навигация школьных и организационных вопросов, активный раздел и счётчик, доступная только администраторам. `organizer-pages.tsx`: выровненные заголовок, фильтры и карточки обращений. Данные и права не меняются. Проверка размеров/переходов и снимки: `e2e/organizer-layout.spec.ts`.

Блок «Помощь» в Student `student-profile-page.tsx` и его story-композиции `pages.tsx` использует flex-wrap с явными горизонтальными/вертикальными отступами; ссылки на учебные вопросы и почту снабжены иконками, принудительный перенос строки убран.
