...
`classroom_assignment_plans` и `classroom_assignments` хранят для занятия один confirmed-план и максимум один draft/stale со snapshot группы школьника. Draft фиксирует ID и version подтверждённой основы. Первое подтверждение повышает draft, последующие атомарно заменяют assignments при постоянном ID confirmed-плана; no-op только удаляет draft. Алгоритм сохраняет прежнюю допустимую комнату, иначе выбирает наименее заполненную с natural-name tie-break. История школьника читает итог каждого события, а промежуточные пересчёты отдельно не сохраняются. Глобальный rename меняет отображаемое имя, а старое остаётся в audit. Layout change делает plan `stale`; скрытие используемой комнаты немедленно даёт текущим затронутым школьникам `reassigning`, restore не возвращает назначение. `not_applicable` зарезервирован для школьника, которому не нужна очная комната.
