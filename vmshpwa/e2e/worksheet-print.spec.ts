...
test('worksheets print only mounted conditions and expanded learning materials', async ({
  ...
  await expect(page.getByRole('button', { name: 'Показать подсказку', exact: true })).toBeVisible()
  expect(beforeRequests.filter((url) => url.includes('/reveal/'))).toEqual([])
  ...
  await expect(page.getByText('Подсказка для печати: проведите высоту AH.')).toBeVisible()
  await expect(page.locator('.vmsh-material-label')).toHaveText('Подсказка')
  ...
  await expect(page.locator('.vmsh-material-label')).toBeVisible()
  ...
  await page.emulateMedia({ media: 'print' })
  await expect(page.locator('.vmsh-material-label')).toHaveText('Решение')
  await expect(sheet).toHaveCSS('color', 'rgb(0, 0, 0)')
  await expect(draft).toBeHidden()
  await expect(question).toBeHidden()
