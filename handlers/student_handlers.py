...
async def post_problem_keyboard(chat_id: int, student: User, *, blocked=False):
    ...
    if prev_keyboard:
        try:
            ...
            pass
    if not blocked:
        text = f"❓ Сейчас можно сдать следующие задачи:\n" \
               f"Задача 1т.1. Сколько чисел от 179 до 1000 (включительно) делятся на 17?\n" \
               f"Задача 1т.2. Из горячего крана ванна заполняется за 23 минуты, из холодного — за 17 минут. Маша открыла сначала горячий кран. Через сколько минут она должна открыть холодный, чтобы к моменту наполнения ванны горячей воды налилось в полтора раза больше, чем холодной?\n" \
               f"Нужно сдать только ответ в пункт а) и фотографию решения или черновика в пункт б).\n" \
               f"Вопросы можно задавать в любой момент.\n" \
