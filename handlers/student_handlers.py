...
async def prc_problems_selected_callback(query: types.CallbackQuery, student: User):
    ...
    # В зависимости от типа задачи разное поведение
        ...
        text = (
            ...
            f"\n<b>Заходите в zoom-конференцию, идентификатор конференции:"
            f"\n{conf_id}, код доступа: {passcode}</b>. "
            f"\n\nПожалуйста, при входе поставьте актуальную подпись:"
            f"\n<b>Уровень Фамилия Имя</b>:\n<pre>{student.level} {student.surname} {student.name}</pre>"
            f" (<a href=\"{instruction_url}\">инструкция</a>)"
