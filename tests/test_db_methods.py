...
class DatabaseMethodsTest(TestCase):
    ...
    def test_survey(self):
        surv_1_id = self.db.add_survey(SURVEY_TYPES.RADIO, USER_TYPE.STUDENT, True, 'WHAT??', ['One', 'Two'])
        surv_2_id = self.db.add_survey(SURVEY_TYPES.CHECKBOX, USER_TYPE.TEACHER, True, 'Check', ['Either', 'Neither'])
        surv_3_id = self.db.add_survey(SURVEY_TYPES.CHECKBOX, USER_TYPE.STUDENT, True, 'Check', ['Either', 'Neither'])
        surv_2 = self.db.get_survey_by_id(surv_2_id)
        self.assertEqual(surv_2['question'], 'Check')
        # print(surv_2)
        surveys = self.db.get_active_surveys(USER_TYPE.STUDENT)
        self.assertEqual(len(surveys), 2)
        self.assertEqual(surveys[0]['question'], 'WHAT??')
        self.assertEqual(surveys[1]['choices'][0]['text'], 'Either')
        # print(surveys)
        self.db.update_survey_result(13, surv_2_id, [123, 125])
        self.db.update_survey_result(13, surv_2_id, [123, 188])
        self.assertEqual(self.db.get_survey_result(13, surv_2_id), [123, 188])
