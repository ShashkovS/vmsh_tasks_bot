...
class DatabaseMethodsTest(TestCase):
    ...
    def test_survey(self):
        surv_1_id = self.db.add_survey(SURVEY_TYPES.RADIO, True, 'WHAT??', ['One', 'Two'])
        surv_2_id = self.db.add_survey(SURVEY_TYPES.CHECKBOX, True, 'Check', ['Either', 'Neither'])
        surv_3_id = self.db.add_survey(SURVEY_TYPES.CHECKBOX, True, 'Check3', ['Either3', 'Neither3'])
        surv_2 = self.db.get_survey_by_id(surv_2_id)
        self.assertEqual(surv_2['question'], 'Check')
        all_student_ids = [s['id'] for s in test_students]
        self.assertIsNone(self.db.get_active_survey(test_students[0]['id']))
        self.assertListEqual([], self.db.get_survey_assigns(surv_1_id))
        self.db.assign_survey(surv_1_id, all_student_ids)
        self.assertListEqual(all_student_ids, self.db.get_survey_assigns(surv_1_id))
        self.assertIsNotNone(self.db.get_active_survey(test_students[0]['id']))
        stud_surv = self.db.get_active_survey(test_students[0]['id'])
        self.assertEqual('WHAT??', stud_surv['question'])
        self.assertIsNone(self.db.get_active_survey(test_teachers[0]['id']))
        self.db.update_survey_result(13, surv_1_id, [123, 125])
        self.db.update_survey_result(13, surv_1_id, [123, 188])
        self.assertEqual([123, 188], self.db.get_survey_result(13, surv_1_id))
        self.db.disable_survey(surv_1_id)
        survey = self.db.get_active_survey(USER_TYPE.STUDENT)
        self.assertIsNone(survey)
        self.assertListEqual([], self.db.get_survey_assigns(surv_2_id))
        self.assertListEqual([], self.db.get_survey_assigns(surv_3_id))
        self.db.assign_survey(surv_3_id, all_student_ids)
        half_students_id = all_student_ids[::2]
        left_students_id = list(set(all_student_ids) - set(half_students_id))
        self.db.assign_survey(surv_2_id, half_students_id)
        self.assertListEqual([], self.db.get_survey_assigns(surv_1_id))
        self.assertListEqual(half_students_id, self.db.get_survey_assigns(surv_2_id))
        self.assertListEqual(left_students_id, self.db.get_survey_assigns(surv_3_id))
        # print(f'{self.db.get_survey_assigns(surv_1_id)=} {[]=}')
        # print(f'{self.db.get_survey_assigns(surv_2_id)=} {half_students_id=}')
        # print(f'{self.db.get_survey_assigns(surv_3_id)=} {left_students_id=}')
        stud_surv_2 = self.db.get_active_survey(half_students_id[0])
        stud_surv_3 = self.db.get_active_survey(left_students_id[0])
        # print(f'{stud_surv_2=}')
        # print(f'{stud_surv_3=}')
