...
-- A season may start with a real diagnostic lesson numbered 0. Keep it a
-- normal course/group lesson so upload, submissions, review and publication
-- exercise the same production path as later lessons.
pragma legacy_alter_table = on;

alter table course_lessons rename to course_lessons_before_zero;
...
select id, course_id, lesson_number, title, created_by_user_id,
       updated_by_user_id, created_at, updated_at, version
from course_lessons_before_zero;

drop table course_lessons_before_zero;

create index course_lessons_course_number_idx
    on course_lessons (course_id, lesson_number, id);

alter table student_lesson_metrics rename to student_lesson_metrics_before_zero;
...
select run_id, student_user_id, lesson_number, group_id, simple_strength,
       complex_strength, max_complex_strength, solved_items, total_items
from student_lesson_metrics_before_zero;

drop table student_lesson_metrics_before_zero;
